...
model_wise_latencies = {}
model_wise_confidences = {}
model_wise_energy = {}
...
def register_node(client_socket):
    ...
    if model_name not in model_wise_latencies:
        model_wise_latencies[model_name] = deque(maxlen=100)
    if model_name not in model_wise_confidences:
        model_wise_confidences[model_name] = deque(maxlen=100)
    if model_name not in model_wise_energy:
        model_wise_energy[model_name] = deque(maxlen=100)
...
def get_user_request(client_socket):
    try:
        ...
        model_string = model_decider_sa_latency(request, registered_nodes , embedding_model,model_wise_avg_metrics)
...
def handle_client(client_socket):
    ...
    with client_socket:
        ...
        # Update the model-wise latencies, confidences, and energy consumption
        model_name = user_request['model']
        model_wise_latencies[model_name].append(latency)
        model_wise_confidences[model_name].append(float(evaluation_score))
        model_wise_energy[model_name].append(float(energy_consumption))
        model_wise_avg_metrics = {
            model: {
